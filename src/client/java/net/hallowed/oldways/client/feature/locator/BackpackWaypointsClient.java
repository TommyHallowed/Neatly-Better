...
/**
 * Client-side cache for lodestone waypoints discovered inside the player's
 * Backpacked backpack inventories.  Mirrors {@link EnderWaypointsClient}.
 */
@Environment(EnvType.CLIENT)
public final class BackpackWaypointsClient {

    private static final List<Entry> CACHE = new ArrayList<>();

    public static void register() {
        ClientPlayNetworking.registerGlobalReceiver(OldWaysNetwork.BackpackLodestones.ID,
                (payload, context) -> context.client().execute(() -> {
                    CACHE.clear();
                    for (OldWaysNetwork.LodestoneEntry e : payload.entries()) {
                        ResourceKey<@NotNull Level> dim = ResourceKey.create(
                        ...
                        String lbl = e.label();
                        CACHE.add(new Entry(dim, pos,
                                lbl == null || lbl.isBlank() ? null : lbl, color));
                    }
                }));
    ...
    /**
     * Appends cached backpack lodestone waypoints for the given dimension to
     * the output list.
     */
    public static void appendForDimension(ResourceKey<@NotNull Level> dim, List<ClientWaypoint> out) {
        ...
        for (Entry e : CACHE) {
            ...
            out.add(new ClientWaypoint(
                    e.pos,
                    e.label == null ? java.util.Optional.empty()
                            : java.util.Optional.of(Component.literal(e.label)),
                    net.minecraft.resources.Identifier.fromNamespaceAndPath("old-ways", "lodestone"),
                    e.color == null ? java.util.Optional.empty()
                            : java.util.Optional.of(e.color)
            ));
        }
    }

    private record Entry(ResourceKey<@NotNull Level> dim, Vec3 pos, String label, Integer color) {}
    private BackpackWaypointsClient() {}
}
