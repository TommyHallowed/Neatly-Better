...
public final class ColorHandler {
    ...
    /** Try #RRGGBB in custom name / item name. */
    public static Optional<Integer> getColor(ItemStack stack) {
        if (!ClientConfigManager.allowNameColorCodes()) return Optional.empty();
    ...
    public static Optional<Integer> getColor(String s) {
        ...
        Matcher m = Pattern.compile("#[0-9a-fA-F]{6}").matcher(s);
        if (m.find()) {
            String hex = m.group().substring(1); // drop '#'
            try { return Optional.of(Integer.parseInt(hex, 16)); } catch (NumberFormatException ignored) {}
        }
        return Optional.empty();
    }

    /** Return the text with any single #RRGGBB removed (for nicer labels). */
    public static Optional<Text> removeColorCode(Text text) {
        if (text == null) return Optional.empty();
        return Optional.of(Text.literal(text.getString().replaceAll("( ?)([({<\\[]?)(#[0-9a-fA-F]{6})([)}>\\]]?)", "")));
    }
}
